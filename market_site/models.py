...
class User(db.Model, UserMixin):
	...
	ha_sold = db.Column(db.Float, nullable=True, default=0)
	ha_bought = db.Column(db.Float, nullable=True, default=0)
...
class HardAssetSale(db.Model):
	...
	def sell(self, buyer):
		if buyer.balance >= self.price and buyer != self.hard_asset.owner:
			...
			seller.ha_sold = round(self.price + seller.ha_sold, 2)		# seller sold += price
			buyer.ha_bought = round(self.price + buyer.ha_bought, 2)	# buyer bought += price
...
class CompanyShareSale(db.Model):
	...
	def sell(self, buyer):
		if buyer.balance >= self.price and buyer != self.share.owner:
			...
			seller = self.get_seller()
			seller.la_sold = round(self.price + seller.la_sold, 2)		# seller sold += price
			buyer.la_bought = round(self.price + buyer.la_bought, 2)	# buyer bought += price
